...
| Agent 模拟 | 基于实验配置运行多智能体社会模拟和机制实验；可通过 CLI 或 Claude Code 执行 |
...
| 后端服务 | 提供本地 API，用于 Agent 运行时技能管理、模块探测、回放接口和 API 文档 | 日常使用建议启动；暂未启动时仍可编辑配置和用 CLI/Claude Code 跑实验 |
...
- **FastAPI 后端**：在本地运行，为插件提供技能运行时管理、模块探测、回放和 API 服务。
