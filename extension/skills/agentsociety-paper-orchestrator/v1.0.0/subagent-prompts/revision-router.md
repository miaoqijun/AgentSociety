...
| `figure-plan` | producer (figure_argument_map) → figure-logic-reviewer | `paper-architecture` | figure_argument_map |
| `evidence` | producer (evidence_backlog) → evidence-skeptic | `paper-evidence-expansion` | evidence_backlog |
...
- Route to `evidence` with `missing_analysis`: may dispatch
  `agentsociety-analysis` (cap 2/round). Increment counter on dispatch.
