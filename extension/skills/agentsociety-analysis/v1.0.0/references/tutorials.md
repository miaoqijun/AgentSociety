...
5. use `run-code` to generate `chart_01_trend.py`
...
with sqlite3.connect("sqlite.db") as conn:
    df = pd.read_sql_query(
        """
        SELECT step, metric_value, condition
        FROM metric_table
        ORDER BY step
        """,
        conn,
...
```bash
$PYTHON_PATH .agentsociety/bin/ags.py analysis data-summary --db-path $DB_PATH
$PYTHON_PATH .agentsociety/bin/ags.py analysis run-eda --db-path $DB_PATH --output-dir $OUTPUT_DIR/data --type quick-stats --tables table_a,table_b
```
