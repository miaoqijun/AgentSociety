...
---
name: cognition
description: 从观察与记忆更新 emotion.json 和 intention.json（LLM 驱动，无默认子进程脚本）。
---
```

`name` 与 `description` 会进入选择 catalog；仅 `memory` 等技能在 frontmatter 声明 `script:` 时才会通过 `execute_skill` 跑维护脚本。输入、输出和依赖关系写在 Markdown 正文与 `references/*_schema.json` 中。
