...
__all__ = ["AskContext", "CodeGenRouterV2"]
...
def _build_deterministic_final_answer(
    router: "CodeGenRouterV2", success_data: Dict[str, Any]
) -> str:
...
def _validate_generated_results(results: Dict[str, Any]) -> Tuple[bool, str]:
    status = results.get("status")
    if status not in {"success", "in_progress", "fail", "error"}:
        return (
            False,
            "Generated code must set results['status'] to one of: "
            "success, in_progress, fail, error.",
        )
    summary = results.get("summary")
    if not isinstance(summary, str) or not summary.strip():
        return False, "Generated code must set a non-empty string results['summary']."
    return True, ""
...
class CodeProvider(Protocol):
    ...
    async def get_code(
        self, context: AskContext, router: "CodeGenRouterV2"
    ) -> Optional[str]:
...
class InstructionLogObserver:
    ...
    def __init__(self, router: "CodeGenRouterV2"):
        self._router = router
...
class CacheCodeProvider:
    ...
    async def _compute_embedding(
        router: "CodeGenRouterV2", text: str
    ) -> Optional[np.ndarray]:
        try:
    ...
    @staticmethod
    async def _lookup(
        router: "CodeGenRouterV2", instruction: str, variables: dict
...
2. Store machine-readable outputs in the `results` dictionary.
3. MUST set results['status'] at the end: 'success', 'in_progress', 'fail', or 'error'.
4. MUST set results['summary'] at the end. This is the final natural-language answer returned to the agent.
5. Build results['summary'] with explicit if/elif/else branches based on tool outputs.
6. The summary should be short, scenario-based, and include important ids, time, values, or failure reasons when relevant.
7. Provide semantic print statements to explain what you're doing, but do not rely on print() as the final answer.
...
- NEVER forget to set results['summary'] at the END of your code!
...
- Example code pattern:
  ...
  if isinstance(response, dict) and response.get("status") in ("success", "ok"):
      results["status"] = "success"
      results["summary"] = f"Completed the requested operation. Response id: {{response.get('id')}}."
  elif isinstance(response, dict) and response.get("status") == "in_progress":
      results["status"] = "in_progress"
      results["summary"] = "The requested operation has started and needs to be checked again later."
  elif isinstance(response, dict):
      results["status"] = "fail"
      results["summary"] = f"Could not complete the requested operation: {{response.get('error') or response.get('reason') or response}}."
  else:
      results["status"] = "success"
      results["summary"] = f"Completed the requested operation. Result: {{response}}."
...
Please generate the corrected code:"""
    ...
    @staticmethod
    async def _call_llm(
        router: "CodeGenRouterV2", context: AskContext
...
class InitStage:
    ...
    async def process(self, context: AskContext, router: "CodeGenRouterV2") -> AskContext:
...
class CodeStage:
    ...
    async def _acquire_code(
        router: "CodeGenRouterV2", context: AskContext, retry_only: bool
    ) -> bool:
        providers = (
    ...
    @staticmethod
    def _validate_code_safety(router: "CodeGenRouterV2", code: str) -> Tuple[bool, str]:
        violations = []
        try:
    ...
    async def _execute_code(
        router: "CodeGenRouterV2", code: str, ctx: dict, readonly: bool
    ) -> Dict[str, Any]:
        results = {}
    ...
    async def process(self, context: AskContext, router: "CodeGenRouterV2") -> AskContext:
        ...
        while context.retry_count <= max_retries:
            ...
            valid_results, validation_error = _validate_generated_results(results)
            if not valid_results:
                context.retry_count += 1
                context.previous_code = code
                context.previous_errors.append(validation_error)
                if context.retry_count > max_retries:
                    context.early_return = (
                        context.ctx,
                        f"Generated code produced invalid results after retries: {validation_error}",
                    )
                    return context
                context.code = None
                continue
...
class SummaryStage:
    """从生成代码写入的 results['summary'] 构造最终答案。"""
    ...
    async def process(self, context: AskContext, router: "CodeGenRouterV2") -> AskContext:
        ...
        final_answer = (
            f"[{router.t.strftime('%A')}, {router.t.strftime('%Y-%m-%d %H:%M:%S')}] "
            f"{context.results['summary'].strip()}"
        )
        determined_status = context.results["status"]
...
class CodeGenRouterV2(RouterBase):
    """
    ...
    6. 直接返回生成代码写入 results['summary'] 的最终响应，不再调用 summary LLM
    """
    ...
    def __init__(
        ...
        cache_dir = template_cache_dir or os.path.join(
            Config.HOME_DIR, "codegen_router_v2_cache"
        )
    ...
    async def _run_builtin_observe(self, ctx: dict) -> Dict[str, Any]:
        ...
        if n_tools == 0:
            return {
                ...
                "results": {
                    "observations": {},
                    "status": "fail",
                    "summary": "No observe tools are registered, so no environment state could be observed.",
                },
                "print_outputs": [],
                "output": "",
        ...
        status = "success" if n_ok else "fail"
        if n_ok and errors:
            summary = (
                f"Observed {n_ok} environment item(s), but some observe tools failed: "
                f"{'; '.join(errors)}."
            )
        elif n_ok:
            summary = f"Observed {n_ok} environment item(s): {json.dumps(observations, ensure_ascii=False, default=str)}"
        else:
            summary = f"Failed to observe the environment: {'; '.join(errors)}."
