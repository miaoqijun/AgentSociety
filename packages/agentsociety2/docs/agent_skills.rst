...
Agent Skills 是 ``PersonAgent`` 的能力组织方式。它解决的问题不只是“如何写插件”，而是如何把社会科学中的行为机制拆成可以检查的仿真环节：一个人看到了什么，如何解释这个情境，形成了什么意图，采取了什么行动，又把哪些经历带到后续决策中。

对于研究者来说，这种拆分比把所有行为写进一个巨大类更自然。理论假设往往关心的是机制链条：信息暴露如何影响情绪，社会规范如何改变意图，资源约束如何改变计划，重复互动如何沉淀为记忆。Agent Skills 把这些环节拆成可读、可替换、可调试的模块，使研究者可以逐步修改某个机制，而不是重写整个 agent。
...
这四个技能不是随意拆分的工程模块，而是把一个社会行动者的基本行为链条拆成四个可观察环节：

1. **感知情境**：行动者首先需要知道当前环境发生了什么。
2. **解释情境**：同一事实对不同身份、目标和情绪状态的人意义不同。
3. **形成并执行行动**：意图需要被转化为具体可执行的环境动作。
4. **保留经验**：行动结果和社会互动会影响后续判断，而不是每个 tick 从零开始。

在真实社会科学研究中，研究者往往关心的正是这些环节之间的机制关系：信息暴露如何改变判断，规范压力如何影响意图，资源约束如何改变计划，重复互动如何积累信任或怨恨。把这些能力拆成独立 skill，可以让实验者单独检查和替换某个机制，而不是把所有行为都压进一段不可分解的 LLM prompt。
...
内置技能的理论依据被转化为可执行的工程约定。这里的目标不是声称 agent 精确复刻人类心理，而是让仿真中的行为变化有稳定结构、可解释状态和可复盘证据。

为什么是这四个基础技能
^^^^^^^^^^^^^^^^^^^^^^

``observation``、``cognition``、``plan``、``memory`` 对应的是一个最小但完整的行动闭环：
...
.. list-table::
   ...
   * - 环节
     ...
     - 设计理由
   * - 感知
     - 个体处在特定制度、空间、网络和互动场景中，行为不能只由静态画像决定。
     - 通过 ``observation`` 把环境事实显式写入 workspace，避免 LLM 凭空补全情境。
   * - 解释
     - 社会行动不是对刺激的机械反应，而是经过情绪、价值、目标、规范和控制感的解释。
     - 通过 ``cognition`` 维护情绪和意图状态，让同一环境事实可以因 profile 与历史不同而产生不同意义。
   * - 行动
     - 意图需要落实为可执行行为；复杂目标还需要计划、暂停、恢复和修正。
     - 通过 ``plan`` 把目标转成环境工具调用或多步计划，并把计划状态写成可检查文件。
   * - 经验
     - 社会互动具有路径依赖。关系、承诺、失败和成功会改变后续行动。
     - 通过 ``memory`` 保存值得跨 tick 使用的经验，使 agent 的行为具有历史连续性。

这种拆分也方便实验干预。研究者可以只改变信息暴露而不改认知规则，只改变记忆保持策略而不改环境模块，或者对比“无记忆 / 有记忆”“固定计划 / 可中断计划”等处理组。
...
.. list-table::
   ...
   * - Situated cognition / Embodied cognition
     - 个体认知嵌入具体环境与行动机会中；环境不是背景，而是认知过程的一部分。
     - ``observation`` 每个 tick 从环境取回事实，并把自然语言观察和结构化上下文落盘。
   * - BDI agent model
     - 行为可理解为 beliefs、desires、intentions 之间的动态关系。
     - ``observation`` 更新近似 beliefs，``cognition`` 生成 intention，``plan`` 将 intention 转成 action。
   * - Component Process Model
     - 情绪来自对事件的连续 appraisal，包括新颖性、愉悦性、目标相关性、控制感和规范意义。
     - ``cognition`` 的 appraisal 变量和情绪强度更新。
   * - OCC / 离散情绪建模
     - 情绪标签可由事件、行动者和对象之间的评价关系产生，便于把情绪作为可分析状态。
     - ``state/emotion.json`` 同时保存离散主情绪、强度、valence 和 arousal，便于跨实验比较。
   * - Theory of Planned Behavior
     - 意图由 attitude、subjective norm、perceived behavioral control 共同预测。
     - ``state/intention.json`` 的候选目标评分字段。
   * - Bounded rationality
     - 个体通常在有限信息、有限时间和有限计算能力下做出足够可行的决策，而不是全局最优决策。
     - ``plan`` 支持单步行动、有限轮工具循环和可中断计划，使 agent 在局部上下文中行动。
   * - Dual-process decision making
     - 快速自动加工适合熟悉/低风险情境，慢速审慎加工适合新颖/复杂/冲突情境。
     - ``plan`` 在单步行动与 ``plan_state.json`` 多步计划之间切换。
   * - Prospective memory / Goal management
     - 人会保留未来要完成的目标，并在条件满足时恢复或调整计划。
     - ``plan_state.json`` 记录 ``status``、``resumable``、``resume_conditions`` 等字段。
   ...
   * - Episodic memory / Autobiographical memory
     - 个体会把重要经历、关系、地点和承诺组织为可回忆片段，这些片段影响后续身份与判断。
     - ``state/memory.jsonl`` 保存带时间、重要性和内容的经验记录，区别于框架级 thread 摘要。

为什么不是固定流水线
^^^^^^^^^^^^^^^^^^^^

虽然这四个技能构成了完整行动闭环，``PersonAgent`` 并不强制每个 tick 都按固定顺序执行它们。原因有三点：

* **社会行动具有情境性**：有些 tick 只需要观察，有些 tick 需要立即执行计划，有些 tick 只是维护记忆。固定流水线会制造不必要的上下文和行动。
* **LLM 上下文是稀缺资源**：metadata-first 的选择机制先暴露技能名称和描述，只有被选中时才加载完整 SKILL.md，避免每步塞入所有机制说明。
* **实验需要可干预性**：研究者可以在某个处理组中禁用自定义技能、替换某个脚本或改变技能描述，用来测试某个机制是否影响结果。

因此，内置技能提供的是一组理论上可解释的基础能力，而不是一个僵硬的人类心理流水线。运行时的工具循环负责在具体情境中选择哪些能力真正参与当前 step。
