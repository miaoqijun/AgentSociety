...
* **假设生成**: 从研究问题生成可测试的假设
* **实验设计**: 设计完整的实验配置
* **论文撰写**: 通过外部 ``paper-toolkit`` 插件生成和检查论文工作区
* **数据分析**: 分析实验数据并生成报告
...
* **paper-toolkit** - 外部插件，提供论文模板、证据图、检查器、LaTeX 组合和编译工具
...
论文生成、检查、LaTeX 组合和编译由外部 ``paper-toolkit`` 插件提供。
AgentSociety 主仓库保留实验、文献、假设和分析能力；论文工作区的确定性工具、
模板、证据图和检查器位于独立插件中。

典型命令包括 ``paper init``、``paper scan``、``paper template expand``、
``paper evidence validate``、``paper check all``、``paper compose assemble-latex``
和 ``paper compile-once``。
