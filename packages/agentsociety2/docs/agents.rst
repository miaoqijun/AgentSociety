...
但这种范式的实际使用仍然有较高技术门槛。一个完整实验通常要求研究者把高层次的社会科学想法转化为可执行代码：定义 agent 画像、行为逻辑、环境工具、实验步骤、运行配置和分析产物。这个过程依赖较强的编程能力，也要求研究者理解仿真基础设施、状态持久化、LLM 调用和结果回放等工程细节。因此，很多有价值的研究问题很难快速落地为可运行实验。

AgentSociety 2 中的 **AI Social Scientist** 试图在研究想法与可执行仿真之间提供一个交互式编排层。研究者可以更多关注研究问题、理论机制和实验逻辑；系统则辅助完成实验结构化、配置生成、运行管理和结果分析。``PersonAgent`` 就是这个流程中面向“人”的默认智能体抽象：它把人物画像、行为能力、工作记忆和环境互动组织成可检查、可替换、可复盘的运行单元。

本部分先从研究者如何理解 ``PersonAgent`` 开始，再介绍需要调参、扩展或调试时会用到的工程细节。


从研究想法到人物智能体
----------------------

在社会科学实验中，研究者通常不会一开始就想“我要写一个 Python 类”。更自然的起点是：

* 谁是研究对象？他们有什么身份、偏好、资源和约束？
* 他们会观察到什么环境信息？
* 他们如何形成情绪、意图、计划或社会判断？
* 哪些事件值得记住，哪些行为需要被记录和比较？

``PersonAgent`` 对应的正是这组问题。它不要求研究者把人的全部行为写成一个巨大函数，而是把人物拆成三个可理解的层次：``profile`` 描述相对稳定的身份与背景，``workspace`` 保存仿真过程中逐步变化的状态，``skills`` 表达这个人物可以调用的行为能力。

这样做的意义是把“建模人的行为”从一次性编码任务，变成一个可逐步修订的研究过程。研究者可以先用自然语言和配置表达理论假设，再通过 skills、环境模块和 replay 逐步检查：agent 是否看到了正确的信息，是否形成了合理意图，是否按预期行动，以及这些微观行动是否支持或挑战原始假设。
...
.. list-table::
   ...
   * - 字段
     - 说明
   * - ``model``
     - LiteLLM 支持的模型名 / model id；``gpt-5.5`` 只是 OpenAI 示例，也可以使用 Anthropic 或其它 LiteLLM 支持的 provider 模型名。
   * - ``context_window``
