...
def test_experience_memory_commands_parse(tmp_path):
    ...
    guidance = parser.parse_args(["guidance", "--topic", "charts"])
    assert guidance.command == "guidance"
    assert guidance.topic == "charts"

    template = parser.parse_args(["payload-template", "--name", "analysis_plan"])
    assert template.command == "payload-template"
    assert template.name == "analysis_plan"

    scaffold = parser.parse_args(["chart-scaffold"])
    assert scaffold.command == "chart-scaffold"
...
def test_analysis_guidance_and_templates_are_available(tmp_path, capsys):
    guidance = _run_harness_cli(capsys, "guidance", "--topic", "attestation")
    assert "frame" in guidance["guidance"]["rubric_keys"]

    chart_guidance = _run_harness_cli(capsys, "guidance", "--topic", "charts")
    assert "quality_rules" in chart_guidance["guidance"]

    template = _run_harness_cli(
        capsys, "payload-template", "--name", "analysis_plan"
    )
    assert template["template"]["target_tables"]

    scaffold = _run_harness_cli(capsys, "chart-scaffold")
    assert 'matplotlib.use("Agg")' in scaffold["scaffold"]
